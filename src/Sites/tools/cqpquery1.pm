...
# first, collect information for sorting lines
  ...
  #output starts, needs formatting!!!! EMILIANO
	...
	if (($corpusname,$cpos,$matchend)=/^(.+)\.(\d+)\.(\d+)$/) {
	    ...
	    if ($curlang eq 'ar') { # treatment of Arabic: right to left
		($left,$right)=($right,$left);
	    };

		## begin: insert segment-level presentation of the concordance lines here : 2024-09-17
...
sub makecorpusattrs {
	...
	$segmentattr = $clcorpus->attribute("s",'s'); # added segment attribute 2024-09-17
